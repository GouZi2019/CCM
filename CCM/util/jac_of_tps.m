function jac_det = jac_of_tps(src_points, tgt_points, check_point, p)
dim = size(src_points,1);
npnts = size(src_points,2);

if nargin<4
    tps_param = tpaps_nd(src_points,tgt_points);
else
    tps_param = tpaps_nd(src_points,tgt_points,p);
end
tps_centers = src_points;

W = tps_param(:,1:npnts);
A = tps_param(:,npnts+1:npnts+dim);

vec_matrix = (check_point - tps_centers)';
vec_square = sum(abs(vec_matrix).^2,2);
vec_t = log(vec_square) + 1;
V = 2.*vec_t.*vec_matrix;
V(isnan(V)) = 0;
...
if nx==m+1 % simply return the interpolating plane
    param = [zeros(dy,m+1), y/(R(1:mp1,1:mp1).')];
%     st = stmak(x,[zeros(dy,3), y/(R(1:mp1,1:mp1).')],'tp00');
    ...
    if nargin==3&&~isempty(p)&&p==0 % get the linear least squares polynomial:
        param = [zeros(dy,nx), (y*Q1)/(R(1:mp1,1:mp1).')];
...
%         st = stmak(x,[coefs1,coefs2],'tp00');
        ...
        if nargin<3||isempty(p) % we must supply the smoothing parameter
            ...
            if m==2
                ...
                p = 1/(1+mean(diag(q'*stcol3d(xx,xx)*q)));
            end
        end
...
for rr = 1:nx
    for cc = 1:ny
        colmat(rr,cc) = sum( (x(:,rr) - y(:,cc)).^2 ); %R^2 
%         trmatrix(cc,rr) = trmatrix(rr,cc);
    end
end
%calculate kernel function R
colmat = colmat.* log(colmat);
colmat(isnan(colmat)) = 0;
end